...
class AddressManager(models.Manager):
	...
	#添加收获地址
	def add_one_address(self, passport_id, recipient_name, recipient_addr, zip_code, recipient_pthone):
		#判断用户是否有默认收获地址
...
class Address(BaseModel):
	...
	recipient_pthone = models.CharField(max_length=11,verbose_name='联系电话')
...
class PassportManager(models.Manager):
	def add_one_passport(self, username, password, email):
		# 添加一个账户信息
		passport = self.create(username=username, password=get_hash.get_hash(password), email=email)
		return passport

	def get_one_passport(self,username,password):
		#根据用户名密码查找账户的信息
		try:
			passport = self.get(username=username, password=get_hash.get_hash(password))
		except self.model.DoesNotExist:
			# 账户不存在
			passport = None
		return passport
